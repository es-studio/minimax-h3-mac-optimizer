#!/usr/bin/env python3
"""Queue MiniMax H3 T2V with INT4 32B TE layer-stream + INT4 DiT full load.

No ClipProj: CLIPLoader type is `minimax`. The ~15GB INT4 TE is streamed
one LM layer at a time (same hook as INT8). After encode the fast-disk hook
drops the TE. INT4 DiT (~11.3GB) then full-loads — auto block-stream is
INT8-only; INT4 already sampled 20/20 at ~173s/step on 24GB.
"""
...
OUT = Path("/Users/eunsung/minimax/t2v_int4_te_dit.json")
...
prompt = {
    ...
    "92": {
        ...
        "inputs": {
            "video": ["12", 0],
            "filename_prefix": "video/MiniMax_H3_T2V_int4_te_dit_864x480_5s",
            "format": "auto",
            "codec": "auto",
        },
    },
        ...
        json.dumps(
            {
                ...
                "settings": (
                    f"T2V 864x480 5s {STEPS}steps INT4 32B TE layer-stream "
                    "+ INT4 DiT full load"
                ),
                "dit": DIT,
                "te": TE,
