#!/usr/bin/env python3
"""Queue MiniMax H3 T2V with ClipProj 4B TE + INT8 DiT block streaming.

Same 864x480 / 5s / 20 steps as the INT4 run. The 20GB INT8 file is not
resident: each of the 50 DiT blocks (~387MB) is pread, computed, then dropped.
"""
...
OUT = Path("/Users/eunsung/minimax/t2v_int8_blockstream.json")
...
prompt = {
    ...
    "92": {
        ...
        "inputs": {
            "video": ["12", 0],
            "filename_prefix": "video/MiniMax_H3_T2V_int8_blockstream_864x480_5s",
            "format": "auto",
            "codec": "auto",
        },
    },
        ...
        json.dumps(
            {
                ...
                "settings": "T2V 864x480 5s 20steps ClipProj 4B + INT8 block-stream",
                "dit": DIT,
